...
opt.num_epochs = 2
opt.print_freq = 10
opt.train_percentage=0.05 # to be used for debugging with low number of samples
...
for epoch in range(opt.num_epochs):
    ...
    if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval== 0:
       ...
       torch.save(model.state_dict(), "saved_models/%s/generator_%d.pth" % (opt.experiment_name, epoch))
